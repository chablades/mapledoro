...
export default function SetupFlowScreen({ model, actions }: SetupFlowScreenProps) {
  ...
  // quick flow hasn't been completed yet. Once it has, step 0 shows the character's profile
  // instead (see getActiveScreenId in PreviewSetupPane.tsx), so the "Setup selection" menu
  // entry would be a misleading label there and is omitted.
  ...
  // per-step-id map every render, not from whichever step happens to be current. A step's
  // draft data, and therefore its validity, is shared across flows and outlives navigating
  // away from it, so the block has to hold even after backing out or switching flows, not
  // only while sitting on the broken step.
  ...
  const jumpSteps = visibleSteps.map((s) => {
    ...
    // The broken step's own substeps before the broken one stay navigable. Only that substep
    // onward within it, and every step strictly after it, are blocked, since jumping between
    // substeps of the step being fixed skips past nothing.
